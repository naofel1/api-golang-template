...
func (ac *AdminCreate) check() error {
	...
	if _, ok := ac.mutation.Pseudo(); !ok {
		return &ValidationError{Name: "pseudo", err: errors.New(`ent: missing required field "Admin.pseudo"`)}
	}
...
func (ac *AdminCreate) createSpec() (*Admin, *sqlgraph.CreateSpec) {
	...
	if value, ok := ac.mutation.Pseudo(); ok {
		_spec.SetField(admin.FieldPseudo, field.TypeString, value)
		_node.Pseudo = value
	}
	if value, ok := ac.mutation.PasswordHash(); ok {
		_spec.SetField(admin.FieldPasswordHash, field.TypeBytes, value)
